...
data_dir = r"E:/HKUSTGZ/AAM/config/calibration/right_camera/data"

endpose_csv_path = os.path.join(data_dir, "endpose2.csv")
image_dir = os.path.join(data_dir, "image2")
camera_config_path = os.path.join(data_dir, "camera_config.npy")

output_ecT_csv = os.path.join(data_dir, "ecT.csv")
output_ecT_npy = os.path.join(data_dir, "ecT.npy")
output_used_pairs_csv = os.path.join(data_dir, "used_pairs.csv")

# 棋盘格参数：必须按你的标定板修改
PATTERN_SIZE = (9, 6)       # chessboard inner corners: (cols, rows)
SQUARE_SIZE_M = 0.023       # square size in meter
...
def main():
    ...
    # ------------------------------------------------------------
    # Load robot end poses
    # ------------------------------------------------------------
    base_T_ee_list = load_endposes(endpose_csv_path)

    print(f"\nLoaded robot endposes: {len(base_T_ee_list)}")

    # ------------------------------------------------------------
    # Load images
    # ------------------------------------------------------------
    image_paths = []
    for i in range(1, len(base_T_ee_list) + 1):
        p = os.path.join(image_dir, f"{i}.png")
        image_paths.append(p)

    print(f"Expected images: {len(image_paths)}")
    ...
    for idx, image_path in enumerate(image_paths):
        ...
        used_pairs.append(idx + 1)

        print(f"[OK] Use pair: image {idx + 1}.png")
