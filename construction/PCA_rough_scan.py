...
def define_scan_pose(scan_points, centroid_base, radius, height, rx=0, ry=0):
    ...
    for i in range(scan_points):
        ...
        x_axis = np.cross(z_axis, world_z)
        ...
        # 矩阵的列向量分别为 X, Y, Z 轴
        bcR = np.column_stack((x_axis, y_axis, z_axis))
        # 检查行列式，确保是右手坐标系
        if np.linalg.det(bcR) < 0:
            bcR[:, 0] *= -1 # 翻转 X 轴修正
        ...
        beT = bcT @ np.linalg.inv(ecT)
        Pe = beT[:3, 3]
        beR = beT[:3,:3]

        # --- 4. 转换为 RPY (如果需要) ---
        # 欧拉角，旋转矩阵转换为rx,ry,rz
        rx, ry, rz = rotation_matrix_to_rpy(beR)
...
ecT = np.array([[0,0.9397,0.342,-0.069],
       [-1,0,0,0.044],
