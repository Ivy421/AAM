...
PROJECT_ROOT = os.getenv('/home/smmg/AAM')
...
def synchron_piper(arm_name, camera_syn_endpose_path):
    ...
    endpose_info = piper.get_endpose()
    if endpose_info is None:
        piper.disconnect()
        raise RuntimeError("Failed to read Piper endpose.")

    x_mm, y_mm, z_mm, rx_deg, ry_deg, rz_deg = endpose_info
    data = [
        {"x": round(float(x_mm) * 1000.0)},
        {"y": round(float(y_mm) * 1000.0)},
        {"z": round(float(z_mm) * 1000.0)},
        {"rx": round(float(rx_deg) * 1000.0)},
        {"ry": round(float(ry_deg) * 1000.0)},
        {"rz": round(float(rz_deg) * 1000.0)},
    ]
...
if __name__ == "__main__":
    capture('/home/smmg/AAM/data/runs/20260801_153620/pickplace/','apriltag',1,1,0,1,True)
