...
def define_pregrab_endpose(endpose):
    ...
    if best is None or not best["feasible"]:
        print("Warning: no reachable pregrab pose found on the continuous line.")
        return None
    ...
    print("Selected pregrab endpose:", pregrab_endpose)
...
def define_prefix_endpose(endpose, fix_normal_path, fix_points_path):
    ...
    best = None
    print("\n========== Pre-fix continuous square search ==========")
    print("n_fix_plane:", normal)
    print("outermost fix point [m]:", anchor)
    print("first square corner [m]:", first_corner)
    print("square XY min/max [m]:", square_min_xy, square_max_xy)
    ...
    if best is None or not best["feasible"]:
        print("Warning: no reachable prefix pose found on the continuous square.")
        return None
    ...
    print("Selected prefix joint degrees:", np.round(np.rad2deg(best["q"]), 3))
    print("Selected prefix endpose:", prefix_endpose)
...
b_obj_grab_t = np.array([[printerx],[printery],[printerz + full_box_z_height/2 ]])  ## 固定值代表打印盘中心相对base的位置
