...
with open('E:/HKUSTGZ/AAM/construction/data/coarse_scan/corner_mapping_result.json','r',encoding='utf-8') as f:
    data = json.load(f)
...
# 对 defect mask 做平滑
defect_close_iter = 2
pcd_raw = o3d.io.read_point_cloud("E:/HKUSTGZ/AAM/construction/data/fine_scan/fine_fuse.pcd")
points_raw = np.asarray(pcd_raw.points)
plane1, plane1_pcd,  _ = find_plane(pcd_raw, voxel_size=0.001, distance_threshold=0.002)
...
boundary_jump_th = 0.01   # 3 mm，可调 0.002~0.004
median_window = 20
...
if CORNER_MODE == 'max_u_min_v':
    ...
    u_line_clean = keep_main_cluster(u_line, eps=0.002, min_samples=15)
    u_line = smooth_line(u_line_clean, window=41, polyorder=2) #window=41

    v_line_clean = keep_main_cluster(v_line, eps=0.002, min_samples=15)
    v_line = smooth_line(v_line_clean, window=41, polyorder=2)  #v_line_clean
...
# 保存每一层 barrier mask，后续用于胶水涂抹区域计算
barrier_raw_masks = []
...
while z <= max_search_depth:
    ...
    if not low_points:
        ...
        # 原始 barrier：当前 z 层的缺陷截面点
        raw_barrier_mask = np.zeros((H, W), dtype=bool)
        raw_barrier_mask[sy[svalid], sx[svalid]] = True
        raw_barrier_mask = raw_barrier_mask & domain_mask

        # 处理后的 barrier：实际参与 flood fill 的连续阻挡边界
        ...
        # 只保存面向缺失空间 / repair side 的外侧 contact margin。
        # barrier_mask 本身可能是大面积区域；repair_mask_z 是从缺角 flood fill 得到的缺失空间侧。
        # 取 barrier_mask 中与 repair_mask_z 相邻的一圈，作为真正的 fix/contact surface。
        fix_surface_mask = barrier_mask & ndimage.binary_dilation(
            repair_mask_z,
            iterations=1
        )
        fix_surface_mask = fix_surface_mask & domain_mask

        # 保存当前层 barrier/fix mask
        # raw 仍保存原始截面点；processed 改为保存外侧 contact margin，而不是整块 barrier 体积区域。
        barrier_raw_masks.append(raw_barrier_mask.copy())
        barrier_processed_masks.append(fix_surface_mask.copy())
        ...
        # 同时保存 fix surface mask 对应的 3D 点，方便后续直接可视化/计算涂胶路径
        barrier_uv = mask_to_uv_points(
            fix_surface_mask,
            u_min,
            v_min,
            grid_res
        ...
        if len(barrier_uv) > 0:
            barrier_z_arr = np.full(len(barrier_uv), z)
            barrier_pts_3d = uvz_to_3d(
                barrier_uv,
                barrier_z_arr,
                origin,
                u_axis,
            ...
            barrier_points_all.append(barrier_pts_3d)
...
u_side_pcd.points = o3d.utility.Vector3dVector(side_points[u_side_name])
u_side_pcd.paint_uniform_color([0.0, 1.0, 0.0])  

v_side_pcd = o3d.geometry.PointCloud()
v_side_pcd.points = o3d.utility.Vector3dVector(side_points[v_side_name])
...
completion_out_dir = "E:/HKUSTGZ/AAM/completion/data/completion_result/depression"
...
# 保存 barrier masks
if len(barrier_processed_masks) > 0:
    ...
    barrier_raw_masks_arr = np.zeros((0, H, W), dtype=bool)
    barrier_processed_masks_arr = np.zeros((0, H, W), dtype=bool)

np.savez(
    completion_out_dir + "/fix_mask.npz",
    ...
    # 当前层 barrier 是否被 flood fill 判定有效
    ...
    top_defect_mask=defect_mask
)

# 保存 barrier 的 3D 点云，方便直接可视化
if len(barrier_points_all) > 0:
    barrier_points_all = np.vstack(barrier_points_all)
else:
    barrier_points_all = np.empty((0, 3))

barrier_pcd = o3d.geometry.PointCloud()
barrier_pcd.points = o3d.utility.Vector3dVector(barrier_points_all)
barrier_pcd.paint_uniform_color([1.0, 0.5, 0.0])

o3d.io.write_point_cloud(
    completion_out_dir + "/fix_points.pcd",
    barrier_pcd
)
