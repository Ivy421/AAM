...
def save_file(save_dir, point_file, points_collection, bcT_collection, png_seq_file = None, png_names = None):

    np.savez(
        os.path.join(save_dir, point_file),
        points_collection = points_collection,
        bcT_collection = bcT_collection
    )
    if png_seq_file is not None:
        with open (save_dir + png_seq_file,'w', encoding='utf-8') as f:
            json.dump(png_names,f)
    
    return

def extract_points(data_dir, save_dir, extracted_point_file_name, png_seq_file_name = None ):
    ...
    config_dir = 'E:/HKUSTGZ/AAM/config/calibration/right_camera/'
    ecT = np.load(config_dir + 'ecT.npy')
    ...
    for name in png_names:
        ...
        cam_config_path =  config_dir +  'camera_config.npy'
        points, bcT = frame_pointcloud_generation(image_path,depth_path,endpose_path, cam_config_path, ecT)
        if len(points) == 0:
    ...
    save_file(save_dir,extracted_point_file_name,points_collection, bcT_collection, png_seq_file_name )
    return 
