...
import cv2
from AI_models.qwen import qwen3_inference

# ============================================================
# 路径配置
# ============================================================

FRAME_DIR = str(PROJECT_ROOT / "construction" / "data" / "coarse_scan")

# 你要标注的 RGB 图
# 建议先手动指定为和点云对应的那一帧 RGB
IMG_SEQ = os.path.join(FRAME_DIR, "coarse_png_sequence.json")
TARGET_IDX = os.path.join(FRAME_DIR, "coarse_icp_result.json")
TEXT_PROMPT = 'a dark wooden plank with chipped corner'
# 融合后的目标点云
PCD_PATH = os.path.join(FRAME_DIR, "coarse_fuse.pcd")

CAMERA_CONFIG_PATH = str(PROJECT_ROOT / "config" / "calibration" / "right_camera" / "camera_config.npy")

TRANSFORMATION_PATH = os.path.join(FRAME_DIR, "coarse_point_result.npz")

# 输出标注图
OUT_PATH = os.path.join(FRAME_DIR, "label_corner.png")
MAPPING_PATH = os.path.join(FRAME_DIR, "corner_label_mapping.json")
INFERENCE_RESULT_PATH = os.path.join(FRAME_DIR, "corner_mapping_result.json")
...
def configure_paths(args):
    global FRAME_DIR, IMG_SEQ, TARGET_IDX, PCD_PATH, CAMERA_CONFIG_PATH
    global TRANSFORMATION_PATH, OUT_PATH, MAPPING_PATH, INFERENCE_RESULT_PATH, TEXT_PROMPT

    if args.run_dir:
        FRAME_DIR = str(Path(args.run_dir) / "construction" / "coarse_scan")
        CAMERA_CONFIG_PATH = str(Path(args.run_dir) / "camera_config.npy")

    if args.coarse_scan_dir:
        FRAME_DIR = str(args.coarse_scan_dir)
...
def build_defect_corner_prompt(
    ...
    text_prompt = (
        f"输入的图片中存在一个{object_name}。图片2给这个{object_name} 的四个角点打标签：A,B,C,D。你需要关注的缺陷是物体转角附近的局部断裂或不平整边缘。返回距離缺陷位置最近的标签，只需要输出字母标签，其他不要输出。"
    )
...
def main():
    ...
    with open(TARGET_IDX,'r', encoding='utf-8') as f:
        icp_meta = json.load(f)    
    target_idx = icp_meta['target_index']  
    png = png_seq[target_idx]
    
    RGB_PATH = FRAME_DIR+ '/'+png +'.png'
    ...
    frame_data = np.load(TRANSFORMATION_PATH,allow_pickle = True)
    bcT = frame_data['bcT_collection'][target_idx]
...
#
    ...
    ######################### inference ######################### 
    messages = build_defect_corner_prompt(RGB_PATH, OUT_PATH, TEXT_PROMPT )
    raw_response = qwen3_inference(messages, model_name ='qwen3.7-plus' )
