...
OUTPUT_TRANSFORM_NPZ = COARSE_DIR / "coarse_icp_transforms.npz"
...
def configure_paths(args):
    ...
    global OUTPUT_FUSED_PCD, OUTPUT_RESULT_JSON, OUTPUT_TRANSFORM_NPZ, VISUALIZE
    ...
    if args.coarse_point_file:
        COARSE_POINT_FILE = Path(args.coarse_point_file)
    else:
        COARSE_POINT_FILE = COARSE_DIR / "coarse_point_result.npz"
    if args.coarse_seq_file:
        COARSE_SEQ_FILE = Path(args.coarse_seq_file)
    else:
        COARSE_SEQ_FILE = COARSE_DIR / "coarse_png_sequence.json"
    COARSE_SCANPOSE_FILE = COARSE_DIR / "coarse_scanpose.json"
    ...
    OUTPUT_TRANSFORM_NPZ = Path(args.output_transforms) if args.output_transforms else COARSE_DIR / "coarse_icp_transforms.npz"
...
def main():
    ...
    summary = {
        ...
        "filter_rule": {
            ...
            "overlap_distance_threshold_m": OVERLAP_DISTANCE_THRESHOLD,
        },
    ...
    if VISUALIZE:
        ...
        o3d.visualization.draw_geometries([vis], window_name="Coarse ICP fused point cloud")
