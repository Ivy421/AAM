...
# Candidate viewpoints: camera-facing upper quarter sphere
VIEW_SPACING_M = 0.25
ELEVATION_DEG = [5.0, 25.0, 45.0, 65.0]
YAW_MIN_DEG = -90.0
YAW_MAX_DEG = 90.0

# Keep the original fixed tolerance cube
...
def generate_quarter_sphere_views(object_center, p_cam0, radius):
    """
    Generate nominal viewpoint centers on the camera-facing upper quarter sphere.

    Horizontal latitude lines:
        elevation = [5, 25, 45, 65] deg

    For each latitude, the front-half arc length is:
        L = pi * radius * cos(elevation)

    The number of viewpoints is selected so that the mean arc spacing is
    approximately VIEW_SPACING_M. The viewpoints are uniformly distributed
    along the front half of the latitude circle.

    Each viewpoint is only the center of the original fixed-size IK tolerance cube.
    """
    ...
    for elev_deg in ELEVATION_DEG:
        ...
        # Compact sampling with mean spacing close to 0.25 m.
        n_views = max(2, int(np.round(arc_length / VIEW_SPACING_M)))

        # Sample at the centers of equal angular bins over yaw [-90, 90].
        ...
        for i, yaw_deg in enumerate(yaw_values):
            ...
            cube_center = object_center + radius * direction
            ...
            cube_defs.append(
                (name, cube_center, direction, float(yaw_deg), float(elev_deg))
            )
...
def main():
    ...
    # 3. Generate candidate cube centers on the camera-facing upper quarter sphere.
    print("\n========== Candidate viewpoint sampling ==========")
    cube_centers = generate_quarter_sphere_views(object_center, p_cam0, radius)
    print(f"Total candidate cubes: {len(cube_centers)}")
    ...
    scan_records = []
    for name, cube_center, direction, yaw_deg, elev_deg in cube_centers:
        print(
            f"Optimizing cube: {name}, "
            f"yaw={yaw_deg:.2f} deg, elev={elev_deg:.2f} deg, "
        ...
        rec["yaw_deg"] = round(yaw_deg, 4)
        rec["elevation_deg"] = round(elev_deg, 4)
        rec["radius"] = round(radius, 4)
        rec["camera_work_distance_dc"] = round(dc, 4)
        rec["bbox_half_diagonal_db"] = round(db, 4)
    ...
    success_count = sum(int(rec["success"]) for rec in scan_records)

    print("\n========== Coarse scan result ==========")
    print(f"Object center [m]: {object_center}")
    print(f"BBox size [m]: {bbox_size}")
    print(f"BBox diagonal [m]: {bbox_diag:.4f}")
    print(f"db [m]: {db:.4f}")
    print(f"dc [m]: {dc:.4f}")
    print(f"Radius R = dc + db [m]: {radius:.4f}")
    print(f"View spacing target [m]: {VIEW_SPACING_M:.3f}")
    print(f"Candidate cubes: {len(scan_records)}")
    print(f"Reachable cubes: {success_count}/{len(scan_records)}")
